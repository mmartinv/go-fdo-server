...
// TestProtocolStringToCode tests utils.RVProtocolFromString for all known protocols and error cases.
func TestProtocolStringToCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := utils.RVProtocolFromString(tt.input)
			if (err != nil) != tt.wantErr {
				t.Fatalf("utils.RVProtocolFromString(%q) error = %v, wantErr %v", tt.input, err, tt.wantErr)
			}
			if got != tt.want {
				t.Fatalf("utils.RVProtocolFromString(%q) = %d, want %d", tt.input, got, tt.want)
			}
		})
	}
}

// TestProtocolCodeToString tests utils.RVProtocolToString for all known codes and error cases.
func TestProtocolCodeToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := utils.RVProtocolToString(tt.code)
			if (err != nil) != tt.wantErr {
				t.Fatalf("utils.RVProtocolToString(%d) error = %v, wantErr %v", tt.code, err, tt.wantErr)
			}
			if got != tt.want {
				t.Fatalf("utils.RVProtocolToString(%d) = %q, want %q", tt.code, got, tt.want)
			}
		})
	}
}
...
func TestProtocolRoundTrip(t *testing.T) {
	...
	for _, p := range protocols {
		t.Run(p, func(t *testing.T) {
			code, err := utils.RVProtocolFromString(p)
			if err != nil {
				t.Fatalf("utils.RVProtocolFromString(%q) unexpected error: %v", p, err)
			}
			back, err := utils.RVProtocolToString(code)
			if err != nil {
				t.Fatalf("utils.RVProtocolToString(%d) unexpected error: %v", code, err)
			}
			if back != p {
...
// TestMediumStringToCode tests utils.RVMediumFromString for known mediums and error cases.
func TestMediumStringToCode(t *testing.T) {
	tests := []struct {
		...
		{"unknown", "bluetooth", 255, true},
		{"empty", "", 255, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := utils.RVMediumFromString(tt.input)
			if (err != nil) != tt.wantErr {
				t.Fatalf("utils.RVMediumFromString(%q) error = %v, wantErr %v", tt.input, err, tt.wantErr)
			}
			if got != tt.want {
				t.Fatalf("utils.RVMediumFromString(%q) = %d, want %d", tt.input, got, tt.want)
			}
		})
	}
}

// TestMediumCodeToString tests utils.RVMediumToString for known codes and error cases.
func TestMediumCodeToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := utils.RVMediumToString(tt.code)
			if (err != nil) != tt.wantErr {
				t.Fatalf("utils.RVMediumToString(%d) error = %v, wantErr %v", tt.code, err, tt.wantErr)
			}
			if got != tt.want {
				t.Fatalf("utils.RVMediumToString(%d) = %q, want %q", tt.code, got, tt.want)
			}
		})
	}
}
...
func TestMediumRoundTrip(t *testing.T) {
	...
	for _, m := range mediums {
		t.Run(m, func(t *testing.T) {
			code, err := utils.RVMediumFromString(m)
			if err != nil {
				t.Fatalf("utils.RVMediumFromString(%q) unexpected error: %v", m, err)
			}
			back, err := utils.RVMediumToString(code)
			if err != nil {
				t.Fatalf("utils.RVMediumToString(%d) unexpected error: %v", code, err)
			}
			if back != m {
