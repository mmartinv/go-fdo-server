...
run_test() {
  ...
  fingerprint=$(list_device_ca_certs ${owner_url} | jq -r -M '.certs[0].fingerprint')
  ...
  ! send_manufacturer_ov_to_owner "${manufacturer_url}" "${guid}" "${owner_url}" || log_error "Voucher should not be accepted!"
